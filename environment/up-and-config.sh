...
echo 'Create Kafka topic `order` and grant operation rights on that topic to client `kafka-client`'
...
./run-clitools.sh kafka kafka-acls --bootstrap-server kafka:9092 --command-config ./admin/config/adminClient.properties --add --allow-principal User:kafka-client --operation Read --operation Write --topic order
./run-clitools.sh kafka kafka-acls --bootstrap-server kafka:9092 --command-config ./admin/config/adminClient.properties --add --allow-principal User:kafka-client --operation All --resource-pattern-type prefixed --topic inventory-service --group inventory-service
echo -e '\n'
