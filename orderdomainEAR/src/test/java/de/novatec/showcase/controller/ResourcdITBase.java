...
abstract public class ResourcdITBase {
	...
	protected static Order createOrder(Integer customerId, Item item) {
		...
		Builder builder = getAdminUserPasswordProperty(target.request(MediaType.APPLICATION_JSON));
		...
		response = getAdminUserPasswordProperty(target.request()).get();
	...
	protected static Customer createCustomer() {
		...
		target = client.target(CUSTOMER_URL)
				.path(Integer.valueOf(response.readEntity(JsonObject.class).getInt("id")).toString());
		response = getAdminUserPasswordProperty(target.request(MediaType.APPLICATION_JSON_TYPE)).get();
		assertResponse200(CUSTOMER_URL, response);
		return response.readEntity(Customer.class);
	}
	
	protected static Builder getAdminUserPasswordProperty(Builder builder)
	{
	...
	protected static Builder getTestUserPasswordProperty(Builder builder)
	{
