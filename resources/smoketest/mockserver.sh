...
function start_mockserver
{
        ...
        until $(curl --output /dev/null --silent --head --fail -X PUT "http://$MOCKSERVER_HOST:$MOCKSERVER_PORT/status"); do
                sleep 1
        done	
}
...
function stop_mockserver
{
	# wait for still executing calls to mockserver
        sleep 1
        # stop the mockserver
        curl -X PUT "http://$MOCKSERVERR_HOST:$MOCKSERVER_PORT/stop" -H  "accept: */*"
}
